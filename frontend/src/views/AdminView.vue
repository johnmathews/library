...
/** Render a coverage side as "95.2% (gate 85%)" or a dash when unmeasured. */
function formatCoverage(pct: number | null, threshold: number | null): string {
  if (pct === null) return '—'
  const gate = threshold === null ? '' : ` (gate ${threshold}%)`
  return `${pct}%${gate}`
}
...
<template>
  <div id="admin-page" class="max-w-4xl">
    ...
    <section v-show="tab === 'system'" role="tabpanel" data-testid="admin-tab-system">
      ...
      <div v-else-if="system" class="space-y-6">
        ...
        <div :class="cardClass">
          ...
          <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            ...
            <div>
              ...
              <dd class="text-gray-800 dark:text-gray-100 font-mono" data-testid="system-git-sha">
                {{ system.git_sha ?? '—' }}
              </dd>
            </div>
          </dl>
        ...
        <div :class="cardClass">
          ...
          <table class="w-full text-sm" data-testid="system-config">
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700/60">
              <tr v-for="[key, value] in configEntries" :key="key" data-testid="system-config-row">
                <td class="py-2 pr-4 text-gray-500 dark:text-gray-400 align-top whitespace-nowrap">
                  {{ key }}
                </td>
                <td class="py-2 text-gray-800 dark:text-gray-100 font-mono break-all">
    ...
    <section v-show="tab === 'coverage'" role="tabpanel" data-testid="admin-tab-coverage">
      ...
      <div v-else-if="coverage" :class="cardClass">
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div>
            <dt class="text-gray-500 dark:text-gray-400">Backend</dt>
            <dd class="text-gray-800 dark:text-gray-100 font-medium" data-testid="coverage-backend">
              {{ coverage.backend ? formatCoverage(coverage.backend.pct, coverage.backend.threshold) : '—' }}
            </dd>
          </div>
          <div>
            <dt class="text-gray-500 dark:text-gray-400">Frontend</dt>
            <dd class="text-gray-800 dark:text-gray-100 font-medium" data-testid="coverage-frontend">
              {{ coverage.frontend ? formatCoverage(coverage.frontend.pct, coverage.frontend.threshold) : '—' }}
            </dd>
          </div>
          <div>
          ...
          <div>
            <dt class="text-gray-500 dark:text-gray-400">Git SHA</dt>
            <dd class="text-gray-800 dark:text-gray-100 font-mono" data-testid="coverage-git-sha">
              {{ coverage.git_sha ?? '—' }}
            </dd>
          </div>
        </dl>
