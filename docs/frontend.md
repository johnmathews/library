...
`notes` · `metadata` · `comments` · `actions` · `history`), skipping absent cards
(`notes` only for note docs; the series wrapper carries `empty:hidden` so a
seriesless doc reads exactly as before). **Reordering is within a column only** — the
