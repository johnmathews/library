...
- **Dashboard tiles:** `.app-doc-grid` (the responsive 1/2/3/4-column document
  ...
  **abbreviated month names** in tile dates ("17 Sep 2019" rather than
  ...
  Tile description on mobile**) adds `.app-doc-grid--hide-summary-mobile` to the
  grid, which hides each tile's `.app-doc-card__summary` at `<= 640px` (the
  search snippet is left in place — it is query context, not the always-on
  description) to shorten the long/skinny tiles; all of this reverts to the
  roomier desktop styling at `>= 640px`)
  and
