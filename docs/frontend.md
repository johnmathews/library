...
"Details" card is **five independent tiles**: Content, Classification, Sender,
recipient & dates, Financial, and a read-only System tile)
...
and System always show; Classification / Sender, recipient & dates / Financial appear only
...
`metadata-content` · `metadata-classification` · `metadata-parties` ·
...
**Document date** (`date`, the document's own date — value kept for back-compat),
**Due date** (`due_date`), **Expiry date** (`expiry_date`), **Added date**
(`added_date` → `created_at`) and **Last edited** (`last_edited` → `updated_at`).
On a tile every date except the plain document date renders with a short muted
prefix (Due / Expires / Added / Edited) so several dates stay unambiguous;
`created_at`/`updated_at` show their date portion only. Only `date` is enabled by
default. The list API returns all five dates on every item (see api.md §1.10.2).
