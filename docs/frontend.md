...
| `NewNoteView` | `/notes/new` (`note-new`) | In-app **note authoring**: a `#new-note-form` with an `AppInput` title (`#note-title`), an `AppTextarea` markdown body (`#note-body`), and a **live preview** (`[data-testid="note-preview"]`) rendered through the same `marked` + **DOMPurify** sanitise pipeline as the detail-view reader. Save (`#note-save`, disabled until both fields are non-empty) `POST`s to `/api/notes` (`src/api/notes.ts`) and routes to the new note's `document-detail`; API failures surface in `AppErrorSummary`. Reached from the sidebar **New note** link (`data-testid="sidebar-notes-link"`). |
| `AskView` | `/ask` (`ask`), `/ask/:threadId` (`ask-thread`) | **SRE-Agent-style chat layout** (W10): no `max-w-*` cap (the shell owns width); a **persistent `ConversationSidebar`** on the left (with a **thread search** filter, `[data-testid="thread-search"]`, the "New conversation" button moved out of the title row, and a thread list that scrolls internally), and a full-width answer column on the right headed by the shared **`PageHeader`**. On **`lg+`** the page is a **fixed-height flex column** (`#ask-page` is `lg:h-[calc(100dvh-8rem)]` — viewport minus the `h-16` shell header and the `app-page` `py-8`): the **transcript scrolls internally** (`[data-testid="ask-transcript"]`, `lg:flex-1 lg:min-h-0 lg:overflow-y-auto`) and the composer is a `shrink-0` flex sibling **pinned at the bottom** — always visible and, unlike `position:sticky`, never floating over the transcript, so it can't intercept citation clicks. **Below `lg`** (where the 256px sidebar leaves the column too narrow for a fixed-height internal-scroll region — it trapped citation clicks on mobile-webkit) it falls back to **normal page flow**: the transcript flows and the composer sits at the bottom of the page-scrolled column (see `e2e/ask-page-citation.spec.ts`). The transcript is **wide rich-markdown answers, not chat bubbles**: each turn is a card with the question as an `<h2>`, the sanitized markdown answer (`#ask-answer`, `marked` + `DOMPurify`, GFM **tables** styled) filling the width, citation cards in a 2-col grid (each a `RouterLink` to `document-detail`), and a subtle `used_tools`/`cost_usd` meta line. The composer (`[data-testid="ask-form"]`) holds a multi-line `AppTextarea` `#ask-question`, an **Attach image** control (`[data-testid="ask-image-attach"]` → hidden `[data-testid="ask-image-input"]`, up to 5 images previewed as thumbnails `[data-testid="ask-image-preview"]` with a remove button, base64-encoded client-side — W11), and a right-aligned **Send** `AppButton` `#ask-submit` ("Sending…" while pending) → `POST /api/ask` (`src/api/ask.ts`), posting the question + any `images` with the active `thread_id`. An empty-state prompt (`[data-testid="ask-empty"]`) shows before the first question. Errors (notably the 503 "no API key" case) surface in `AppErrorSummary`. A **successful answer never shows an error**: the post-success side effects (record `thread_id`, sync the URL to `/ask/:threadId`, refresh the sidebar) run in `syncThread` *outside* the answer-error `catch`, and a missing/non-numeric `thread_id` or a Vue Router navigation rejection is logged rather than turned into a spurious "Something went wrong" alert. See [ask.md](ask.md). |
...
| `AdminView` | `/admin` (`admin`, `meta.adminOnly`) | **Admin-only** (the `authGuard` redirects non-admins to `/`; the sidebar link is hidden unless `auth.isAdmin`). A `role="tablist"` page with four tabs (`[data-testid="admin-tab-<id>-btn"]` / panels `admin-tab-<id>`), each backed by an `/api/admin/*` endpoint ([api.md §1.18](api.md)): **System** (version + git sha, deployment topology, redacted config table, DB stats), **Architecture** (`architecture.md`/`ingestion.md` rendered through the shared `marked` + DOMPurify pipeline), **Coverage** (backend/frontend % vs gate, or an "unavailable" banner), and **Users** (table with role/active badges + per-row promote/demote/activate and a create-user form; the current user's self-actions are hidden, and the last-admin 409 surfaces inline). See [admin.md](admin.md). |
