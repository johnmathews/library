...
| `AskView` | `/ask` (`ask`), `/ask/:threadId` (`ask-thread`) | **SRE-Agent-style layout** (W10): no `max-w-*` cap (the shell owns width); a **persistent `ConversationSidebar`** on the left (with a **thread search** filter, `[data-testid="thread-search"]`, and the "New conversation" button — moved out of the title row), and a full-width answer column on the right headed by the shared **`PageHeader`**. The transcript is **wide rich-markdown answers, not chat bubbles**: each turn is a card with the question as an `<h2>`, the sanitized markdown answer (`#ask-answer`, `marked` + `DOMPurify`, GFM **tables** styled) filling the width, citation cards in a 2-col grid (each a `RouterLink` to `document-detail`), and a subtle `used_tools`/`cost_usd` meta line. The composer is a **bottom bar** (`[data-testid="ask-form"]`, in normal flow — *not* `position:sticky`, which on short viewports overlaps the last turn's citations and intercepts their clicks; see `e2e/ask-page-citation.spec.ts`): a multi-line `AppTextarea` `#ask-question`, an **Attach image** control (`[data-testid="ask-image-attach"]` → hidden `[data-testid="ask-image-input"]`, up to 5 images previewed as thumbnails `[data-testid="ask-image-preview"]` with a remove button, base64-encoded client-side — W11), and a right-aligned **Send** `AppButton` `#ask-submit` ("Sending…" while pending) → `POST /api/ask` (`src/api/ask.ts`), posting the question + any `images` with the active `thread_id`. An empty-state prompt (`[data-testid="ask-empty"]`) shows before the first question. Errors (notably the 503 "no API key" case) surface in `AppErrorSummary`. See [ask.md](ask.md). |
