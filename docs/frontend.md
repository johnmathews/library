...
2. **`scripts/assert-e2e-ran.mjs`** — parses the Playwright JSON report and fails
   when too few tests executed, or when any skip reason mentions
   `E2E_BASE_URL`. The floor is 40 unless `E2E_MIN_EXPECTED` overrides it, which
   each CI shard does (desktop 30, mobile 25, tablet 22 — set below the observed
   53/44/40 with headroom). They sum to 77, so sharding tightened this gate
   rather than loosening it. Not redundant: a reporter change, a `--grep` that matches
...
`npm run test:e2e` is untouched. Every floor is set well below its run's real
count so adding or removing a test does not red it — they answer "did the suite
