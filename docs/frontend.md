...
- **`panel`** (the default, and the default matters — the view mounts it without
  naming a variant) is the document's single **metadata card**, titled
  **Metadata**: every field group as a titled, **collapsible** section, then the
  facet editor, then a read-only **System** provenance block. One card, one drag
  handle, one place a field lives.
...
**Subsections, spacing and folding.** The panel's sections are
`classification` (language, tags, projects, matters, plus title/kind/summary
when the hero gives them up), `parties` (sender, recipient), `dates`
(document/due/expiry), `facets` and `system` — the ids in
`METADATA_SECTION_KEYS`. Every one is wrapped in the single `SECTION_WRAPPER`
class string (a top rule plus equal padding, dropped on the first section via
`first:`), so the panel has **one** vertical rhythm rather than each group
setting its own margins. Before 2026-09-06 they were bare `min-w-0` divs with no
gap and no rule, so a heading butted straight onto the previous group's last
field and the structure was invisible.

Each heading is a `button` (`[data-testid="section-toggle-<key>"]`, with
`aria-expanded` and `aria-controls`) that folds its section; the body is removed
with `v-if`, not `v-show` — `isVisible()` against a `v-show` ancestor is
unreliable in jsdom and reported a folded row as visible while `aria-expanded`
had already flipped. Collapsed keys persist per-machine in
`library:doc-layout-collapsed-sections-v1` as a **key list**, so "expanded" is
the default for a new and a returning user alike and adding a section needs no
migration; `reconcileCollapsedSections` drops keys that are no longer sections
(`content`, renamed to `classification`, is the first such case). **Reset
layout** expands everything.

Two group notes. `Sender, recipient & dates` was **one** group until
2026-09-06, which put two unrelated kinds of fact under one heading and buried
the dates; it is now `parties` + `dates`. And a group whose fields are *all*
hero-owned is **dropped**, not rendered as a heading over nothing — with the
shipped hero defaults that is exactly what happens to `parties`, since sender
and recipient both sit in the hero.
...
Field values render at one size. `amount` briefly rendered at `text-2xl
font-semibold`, which was right while **Financial** was a card of its own
showing a single large number and wrong once `amount` moved into the hero's stat
grid beside Kind and Sender — and with the Financial group gone that branch could
only ever fire in the hero.
...
`.github/workflows/e2e-nightly.yml` runs `library eval-recall` nightly rather
than in the PR gate, because it needs a warm embedder. It reports and never
gates.

**Owned by `docs/ask.md` §1.2 *Measuring recall*** — the schedule, the
`/health` wait, why it cannot gate, the corpus, the baseline and what the
numbers mean all live there. This section is deliberately a pointer rather
than a second description: the workflow's entire remaining substance is one
CLI command, and the browser journey that justified describing it beside the
e2e suite was deleted with the series stack on 2026-08-31. Two descriptions of
one workflow, neither declared in the other's `Covers:`, is what #137 was
filed about — both could be green and wrong at once.
