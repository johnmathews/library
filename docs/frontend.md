...
| `DocumentDetailView` | `/documents/:id` (`document-detail`) | Two-column on desktop: **metadata card on the left** — a key/value `dl` with inline per-row edit via `App*` inputs (PATCH only the edited field) — and the **preview pane on the right** (browser-native PDF `<iframe>` / `<img>`). The PDF iframe src carries `#view=FitH` so the native viewer fits the page to width (avoids right-edge clipping on narrow viewports); the open-in-new-tab link stays plain. Stacks on mobile, preview first. |
