...
| [`money-facts.md`](money-facts.md) | What an amount means (`amount_kind`, `reference`) and payment identity: the four rules and the veto that collapse one payment documented as two or more documents into a single count, the `payment_overrides` correction table, `library backfill-amounts`, and the REST surface. |
