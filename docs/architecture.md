...
**Seven orphaned series tables.** The legacy series stack — the emergent and
authored series that `/charts` used to draw — was deleted on 2026-08-31 (see
[charts.md](charts.md) for what replaced it, and
[archive/smart-groups.md](archive/smart-groups.md) for the semantic variant). Its
**tables were deliberately left in place**: the code ships first and a follow-up
migration drops them, so a revert of that deploy still has its rows. Nothing in
`src/` reads or writes them today — no ORM class, no query, no job — so treat
them as data awaiting deletion rather than as part of the model:

`series_insights` (0009), `series_membership_overrides` (0015),
