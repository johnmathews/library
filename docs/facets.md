...
case- and accent-insensitively (casefolded, so `Skoda`, `SKODA` and `Škoda`
all resolve to a value keyed `skoda`, or one aliased as `Škoda`, without
needing a separate alias per casing variant — only the comparison folds
case, never the stored key/alias/label text itself). Anything that still
matches nothing becomes `value: null` plus a *suggested label* rather than a
fabricated key. That mapping is pure — no model call — so the closed-set
guarantee is unit-tested without one.
