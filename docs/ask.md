...
1. **Embedding (indexing).** After OCR + extraction, each document's text is
   split into overlapping chunks (`embedding.chunker`) and embedded with
   **bge-m3** (1024-dim, multilingual) by a local **embedder** sidecar
   ...
   `received → ocr → extract → embed → indexed`. Embedding is best-effort — a
   document that fails to embed still reaches `indexed` and stays searchable by
   full-text.
2. **Hybrid retrieval** (`semantic_search`). At query time the question is
   ...
   **Reciprocal Rank Fusion** (RRF, k=60), so exact-term matches (invoice
   ...
   nearest chunk as the citation excerpt.
...
4. **Answer** (`ask.engine`). Claude (`ask_model`, default
   ...
   (document id + title), the tools used, and the estimated cost.
...
1. Citations reference a document and its best-matching chunk ordinal, not a PDF
   page number — OCR text carries no reliable page boundaries.
