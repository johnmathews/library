...
(`semantic_search`, `query_documents`, `get_document`) it
...
11. **Money totals are computed from the model the chart engine replaced, and
    ...
    - **It does not collapse payment identity.** One payment documented as an
      invoice and a receipt is **counted twice** unless the pair has been
      merged. `spend_facts` applies the canonical-document rule that makes this
      right; `sum_amount` does not read `spend_facts`.
