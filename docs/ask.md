...
not from `documents.amount_total`. Three things follow, and each is what makes
an Ask total agree with a chart drawn over the same question:
...
widest gap: seeding either needs a shape `SeedDoc` does not express — a second
document sharing a payment — so covering them is a change to the seeding path,
not just another entry in `SCENARIOS`.

Every seeded amount now carries an `amount_kind`, and that is load-bearing
rather than tidy. Since #136 `sum_amount` totals only the summable kinds, so a
