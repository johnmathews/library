...
recall@k against `library.ask.recall_scenarios`'s synthetic corpus — 90
...
**The first real measurement failed that criterion, and the corpus was made
harder in response.** On 2026-08-27 `library eval-recall --write-baseline` was
run for the first time against real bge-m3 vectors, on the deployed host. It
scored **mean recall@10 = 0.917** — above the 0.90 ceiling. The shape of the
result was as damning as the number: five of the six cases scored exactly
**1.00** and only `breadth-many-mentions` (0.50 at k=12) moved at all. Those
five each expected a *single* document against three or four distractors, so
all four candidates fitted inside the ten-slot cut and the case could not lose
recall no matter how retrieval behaved. The corpus was measuring one case.

The corpus was therefore rebuilt to the shape described above: 53 documents
became 90, each case's cluster grew past its rank cut, and every case except
the control now expects several documents so recall degrades gradually instead
of being all-or-nothing. **That first 0.917 is recorded here as the reason for
the change, not as a baseline** — it describes a corpus that no longer exists,
and no `recall-baseline.json` was committed from it.

**No baseline is committed, and the criterion is still unverified.** The
hardened corpus has not itself been measured — that needs a host with a
reachable embedder, which this arm64 development machine is not (TEI publishes
no arm64 image). Until `library eval-recall --write-baseline` is run against
the 90-document corpus and comes out below 0.90, the acceptance criterion
remains unmet rather than met.
