...
since. **`comparative-uneven-coverage` has therefore never been run at all** —
it was added in the same change as the cross-call rule it exists to measure,
without credentials, so nothing here says the rule works. The claim that the
rule is *needed* rests on reading the prompt and finding no cross-call
obligation in it; the claim that it *helps* is untested and should be treated
as such until someone runs the command above. That is evidence,
