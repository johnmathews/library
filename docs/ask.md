...
**Last verified:** 2026-09-03 — method: **executed**, twice, in both environments, then re-audited. `library eval-recall --write-baseline` on the deployed host (266 archive documents) and `gh workflow run e2e-nightly.yml` on a zero-archive CI stack (runs 33731357224, 33734090629); both returned mean 0.786 with `breadth-many-mentions` 0.17 and `passage-buried-clause` 0.33, and the twelve returned documents were compared marker-by-marker across the two and are identical. The 0.931 clean-stack figure is read from six consecutive nightly logs. Chunk counts, blind floors and the FTS `log2(length+1)` divisor were each measured rather than derived. A separate adversarial pass then re-checked every number in this section against the shipped corpus and `recall-baseline.json` and found eight stale ones (corpus size, case count, which run the baseline holds, the floor range, the crowder count, what the comparability warning compares, the FTS divisor here versus in `search.py`, and the measurement count) — all corrected. Full backend suite 2162 passed, coverage 95%.
