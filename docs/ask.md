...
**Two measurements, two failures, and what they taught.** On 2026-08-27
`library eval-recall --write-baseline` ran for the first time against real
bge-m3 vectors on the deployed host and returned **mean recall@10 = 0.917** —
above the ceiling. Five of six cases scored exactly 1.00: each expected a
*single* document against three or four distractors, so the whole candidate
pool sat inside the ten-slot cut and the case could not lose recall however
retrieval behaved.

The corpus was rebuilt — 53 documents to 90, every case but the control made
multi-target, every cluster grown past its cut — and re-measured. It returned
**0.9028**. Still above the ceiling, and *not one of the four rebuilt cases had
fallen*.

The second failure identified the real defect, and it is arithmetic rather than
authorial. **A cluster must be several times the rank cut, not merely larger
than it.** With `k=10` drawing from a 13-document cluster, ten of the thirteen
candidates come back regardless of ranking, so a retriever choosing at *random*
already scores **0.77**. The measurable range was 0.77 to 1.00 — too narrow for
anything to show through, and any case that got slightly lucky rounded to 1.00.

So the clusters were grown again, to roughly forty documents each (corpus 90 to
201), which puts blind recall at **0.21-0.25** and opens a real range. That
floor is now enforced: `tests/test_recall_scenarios.py` computes each case's
blind recall and fails if any exceeds `MAX_BLIND_RECALL` (0.35). Reverting the
parking cluster to its 13-document shape trips it.

Neither 0.917 nor 0.9028 is committed as a baseline. Both describe corpora that
no longer exist; recording either would anchor future deltas to a fiction.

One reading worth keeping from the second run: those four cases scored 1.00
where blind scores 0.71-0.77, so retrieval was performing clearly above chance.
`date-scoped` is the suggestive one - its notices are identically titled and
their bodies never state a year, so "2022" reaches the embedding only through
#6's context header. That is the first positive signal #6 has produced. It is
not proof: with ten slots drawn from thirteen candidates there was a ~42%
chance of catching all three blind. The wider clusters should make the next run
of that case worth reading.

**No baseline is committed, and the criterion is still unmet.** The
201-document corpus has not itself been measured - that needs a host with a
reachable embedder, which this arm64 development machine is not (TEI publishes
no arm64 image). Until `library eval-recall --write-baseline` runs against it
and comes out below 0.90, the acceptance criterion remains unmet rather than
met. Note the run now seeds and embeds 201 documents, so it makes 201 embedder
calls and takes correspondingly longer than the 53-document original.
