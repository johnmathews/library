...
`coverage` block actually reported. Five scenarios expect disclosure, one per
exclusion reason documented above plus a `needs_review` case; the sixth,
`complete-no-gaps`, is a **control** where nothing was dropped — without it, a
model that hedges in every answer regardless of the facts would score a
perfect pass, which is the opposite of what the eval is for.
...
documents for its own eval-shaped suite. Each run seeds its scenario's
documents into the *configured* database inside one transaction and rolls
that transaction back afterwards, on success or failure alike — nothing
seeded by the eval is ever committed.
