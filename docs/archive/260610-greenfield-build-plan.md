...
# Improvement plan — Library greenfield build

**Status:** historical. The plan for the original greenfield build, written
2026-06-10 before any code existed. It is **not** a description of the system as
built and is not maintained — several units shipped differently, and its `W1`–`W17`
numbering belongs to *this* plan alone. Where it disagrees with the current
docs, the current docs are correct. Restored to `docs/archive/` on 2026-08-11
(from commit `9d48ddc^`); the `Wn` citations in `docs/` resolve against the unit
list below.
