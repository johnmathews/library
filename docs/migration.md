...
**Status:** active. **Last updated:** 2026-09-02 (no prose change: re-stamped because `cli.py`, one of the files this document covers, gained two lines in the disclosure eval's `_seed_scenario` helper (#136), wholly unrelated to the paperless importer; nothing on this page was touched by that change). Earlier (2026-08-29, no prose change: re-stamped because `cli.py`, one of the files this document covers, grew a `backfill-amounts` command (money facts) on the `money-facts` branch, wholly unrelated to the paperless importer; nothing on this page was touched by that change). Earlier (2026-08-28, no prose change: re-stamped because `cli.py`, one of the files this document covers, grew two new commands — `label-archive` and `recipients` — on the `charts-redesign` branch, both wholly unrelated to the paperless importer; nothing on this page was touched by that change). Earlier (2026-08-27): (no prose change: re-stamped because the covered `cli.py` grew `eval-disclosure` on the `feat/disclosure-eval` branch, wholly unrelated to the paperless importer). Earlier (2026-08-13): (no prose change: re-stamped because the covered `cli.py` and `importer/` were touched after the previous stamp; earlier, 2026-08-12: documentation verification sweep: documented `LIBRARY_IMPORT_DEFAULT_OWNER`, the `senders:` block in the dry-run report, and the `id`/`document_type` keys in `extra["paperless"]`; corrected the `user_edited_fields` names). Earlier: 2026-06-11.
**Last verified:** 2026-09-02 — method: `cli.py`'s only change since the previous stamp is a two-line addition inside `_seed_scenario` (`git diff main...HEAD -- src/library/cli.py` shows one hunk at `@@ -1221,6 +1221,8 @@`), passing the disclosure-eval fixture's new `amount_kind` through to the seeded `Document`. `_seed_scenario` is the eval command's helper; the hunk is nowhere near `import_paperless` and calls nothing in `src/library/importer/`. Re-read this document's prose in full and confirmed nothing on this page describes any `cli.py` command other than `library import paperless`, whose five `typer.Option` flags (`url`, `token`, `dry_run`, `no_extract`, `limit`) are unchanged — so **no prose change is required**. Did not re-verify `importer/client.py`, `importer/mapper.py` or `importer/runner.py` in this pass; none changed since the last full verification, and the importer was not run (no paperless instance). Ran `uv run python scripts/check_docs.py --max-violations 0`, clean. Earlier: 2026-08-29 — method: `cli.py`'s only change since the previous stamp is the new `backfill-amounts` command (`git log -p -- src/library/cli.py`, commit `8200423`, one contiguous hunk after the `recipients` command, line 621), which drives `library.money.backfill.run_amount_backfill` and reports `classified`/`empty`/`skipped` counts — it is a standalone money-facts command, added nowhere near and calling nothing in `import_paperless` (line 227) or `src/library/importer/`. Re-read this document's prose in full and confirmed nothing on this page describes `cli.py` commands other than `library import paperless`, so no prose change is needed. Did not re-verify `importer/client.py`, `importer/mapper.py` or `importer/runner.py` in this pass — neither changed since the last full verification. Ran `uv run python scripts/check_docs.py --max-violations 0`, clean. The rest carries forward the earlier verification below unchanged, whose method was: `cli.py`'s only change since the previous stamp is the entire facet-vocabulary work's addition to the file, confirmed to be a single contiguous insertion after `backfill_summaries` (`git diff $(git merge-base main HEAD) HEAD -- src/library/cli.py` shows one hunk, `+@@ -537,6 +537,85 @@`) — the `label-archive` and `recipients` commands. It touches nothing in `src/library/importer/` and adds no lines to `import_paperless` itself. Re-read this document's prose in full against `cli.py`'s current `import_paperless` command and confirmed its five `typer.Option` flags (`url`, `token`, `dry_run`, `no_extract`, `limit`) are present and unchanged in shape; **no prose change required**. Did not re-verify `importer/client.py`, `importer/mapper.py` or `importer/runner.py` against the archive or a live paperless instance in this pass — neither file changed since the last full verification, so this pass carries forward the rest of that check rather than repeating it: the 2026-08-27 partial re-verification (the `eval_disclosure` addition, confirmed unrelated), the 2026-08-13 partial re-verification (commit `1f5e6d4`, mypy narrowing only, confirmed behaviour-preserving) and, before that, the 2026-08-12 full verification, which read `importer/client.py`, `mapper.py` and `runner.py` in full, compared `import_paperless` and all five flags and defaults against `cli.py`, and traced every row of the §1.3.1 mapping table, both §1.4 idempotency keys and each §1.6 limitation to its implementing line; the importer was not run (no paperless instance) in any of these passes.
