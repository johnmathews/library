...
**Status:** active. **Last updated:** 2026-08-13 (no prose change: re-stamped because the covered `cli.py` and `importer/` were touched after the previous stamp; earlier, 2026-08-12: documentation verification sweep: documented `LIBRARY_IMPORT_DEFAULT_OWNER`, the `senders:` block in the dry-run report, and the `id`/`document_type` keys in `extra["paperless"]`; corrected the `user_edited_fields` names). Earlier: 2026-06-11.
**Last verified:** 2026-08-13 — method: partial re-verification, scoped to what the covered code's only change since the last stamp could have invalidated. That change is commit `1f5e6d4`, which touched `cli.py`, `importer/client.py` and `importer/mapper.py` for mypy narrowing; I read its diff in full, confirmed all three edits are behaviour-preserving refactors, and re-checked the three claims that rest on them: §1.3's "verifies the MD5 — one retry on mismatch, then the document is recorded as failed" against `download_original_verified` (still two attempts, then `ChecksumMismatchError`), the five `import paperless` `typer.Option` flags and defaults against `cli.py`, and the mapping table's tag handling against `map_document`'s first-occurrence-wins dedupe. The rest of the document carries forward the 2026-08-12 verification, which read `importer/client.py`, `mapper.py` and `runner.py` in full, compared the `library import paperless` command and all five `typer.Option` flags and defaults against `cli.py`, and traced every row of the §1.3.1 mapping table, both §1.4 idempotency keys and each §1.6 limitation to its implementing line; the importer was not run (no paperless instance).
