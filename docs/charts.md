...
| Ask's document-edit tool (`ask/engine.py`) | a tool-result `error` naming the allocation and saying to clear or replace the lines first — the same wording the `PATCH` route gives, delivered where Ask can act on it |
