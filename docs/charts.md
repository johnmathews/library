...
**Status:** active. **Last updated:** 2026-09-02 (**post-merge review fixes to the same-day change below** (#156 follow-up). The review found the shipped fix's user-facing half did not reach the user: `QuestionDraft.vue` rendered `message` only in its *collapsed* branch, and an unmatchable-only draft can never be collapsed — its first clause always survives — so the "could not be combined" sentence existed on the wire and reached nobody, leaving a raw rule fragment in a list whose every other entry means "not in the vocabulary". §9.1's wire paragraph now states the one-list/two-causes contract explicitly. §11's `split_value` bullet is **corrected**: it claimed non-canonical ids "still resolve", which overstates — they resolve the sender's NAME while the raw string is bound into the SQL, so the panel is correctly labelled and empty, i.e. the guard admits a narrow class of the silence it exists to remove. The empty string is now refused on every axis. Two adjacent gaps recorded rather than fixed: canonical-form sender ids, and `/footer`'s unvalidated `amount_kind`.) Earlier the same day (**the draft path joins the same-facet story, and `/cell` stops answering `0.00` to an impossible question** (issues #127, #155-adjacent). §11 gains two things: the same-facet bullet now names `POST /spending/draft` as a **fifth** rule-taking path that answers differently — it never called `_validate_rule`, so a drafted pair used to reach the client as `expressible: true` with an all-zero preview and then 422 at Save, and `filter_drafted_rule` now drops the second `in` clause and reports it; and a new bullet for `/cell`'s `split_value` shape checks. §9's producer table gains `unmatchable_terms` beside `unknown_terms` and the paragraph saying why the two are kept apart — a dropped same-facet clause names only vocabulary that exists, so reporting it as 'not in the vocabulary' would give a false reason for a real drop. §13's same-facet limit records the draft path's exception. **Membership is deliberately NOT validated on `split_value`**, and §11 says why: a deleted facet value still labels existing rows, so that bucket is drawn and refusing it would reject a cell the chart just rendered. **§13 also loses two limits, both fixed the same day:** re-extraction's partial skip (`currency` and `amount_kind` now withheld with `amount_total`) and `skipped_fields` landing where nobody reads it (now in the `extraction_completed` event and rendered as the timeline's "Left unchanged" line). The second entry records the trap the fix had to avoid — the timeline renders no field list at all, and `extractionBreakdown()` shadows `secondary()` for this event type, so the obvious backend-only or `secondary()`-branch fixes both render nothing.) Earlier the same day (§2's exclusivity claim is **corrected**. It said nothing in the codebase issues SQL against `spend_facts` except `charts/query.py` and `charts/footer.py`; `api/facets.py` has done so since the vocabulary panel shipped, so the sentence was already false, and #136 added a third reader in `structured_query.sum_amount`. §2 now names all four readers and states the property that is actually exclusive and actually load-bearing: the chart engine's own router, `api/spending.py`, builds no SQL, which is what makes `charts/query.py` the single path behind every chart. **Fix round 1:** a first draft of this edit asserted that no *router* builds SQL against the view, which the same pass's own enumeration falsifies — `api/facets.py` is a router and does. `charts/query.py`'s module docstring carried the same false sentence and is corrected with it.) Earlier: 2026-09-01 (the chart rule editor, issue #135. §11 gains `POST /api/spending/preview` and the count reads **thirteen** routes, not twelve; §9 is retitled "Where a rule comes from" and now opens with the two producers of a `Rule` — the draft flow's drop-and-report filter and the editor's 422 — because presenting LLM drafting as the only path would leave a reader looking for `filter_drafted_rule` on a path that does not use it; the old §9 body is now §9.1. §3's model block gains `extra="forbid"` and why it is load-bearing. §11's validation list gains the same-facet refusal, the empty-rule acceptance and the unknown-clause-key 422, and says a rotted chart still loads by id. §13 gains two limits that were previously stated nowhere — a rule edit not rewriting `question_text`, and a chart naming a deleted value refusing when drawn — and its same-facet bullet now records that the shape is refused rather than merely possible.) Earlier: 2026-08-31 (§10.1's Ask row said the tool returns "the same wording the `PATCH` route gives" — true, but nothing enforced it: the two-line refusal was typed out in full in both `api/documents.py` and `ask/engine.py`. It is now a single `spend_lines.AMOUNT_ALLOCATED_REFUSAL` that both pass, and the row says so. The wording itself is unchanged.) Earlier the same day (the legacy series stack was deleted, and three claims here changed together: §10.1's opening ("four of them answer; the fifth does not" → all five answer), its Ask row, and §13's "the fifth `amount_total` writer is unguarded" bullet, now removed — Ask's document-edit tool commits through `spend_lines.commit_allocation` and returns the refusal as a tool result. §11's opening paragraph withdraws the promise to inherit `/api/charts`: the router stays at `/api/spending` deliberately. **Fix round 1:** this stamp's own evidence sentence cited `grep -rn 'api/charts' src/` returning nothing, which the same pass falsified by writing that string into `spending.py`'s docstring; replaced with a check that actually reproduces.) Earlier: 2026-08-30 (spending-view backend, final fix wave: §7.1 documents the `422` on `bucket=excluded` with no `amount_kind`, and §11's `CellOutBody` sentence gains the `label == ""` on an unsplit chart clause. Earlier the same day, Task 8: §11's route table gains `GET /spending/{id}` and `GET /spending/{id}/footer/{bucket}`, now **twelve** routes not ten; new §7.1 documents the footer drill route — `_accounted_rows` as the one shared classify/convert/sign step, the split-document-emits-one-row-per-line shape and why the drill list deduplicates by `document_id` and sums a document's rows, and `FooterDocumentsOut.total` reporting the bucket's full size before paging; §11 gains `DataOut.splits`/`SplitValueOut` and `CellOutBody`'s `label`/`colour`; §13 drops "a sender split emits ids, not names" — fixed by Task 5 — and gains "`unconvertible` has no drill-through" in its place. `migrations/versions/0037_split_colour.py` joins **Covers**. The live-archive verification of the pre-existing engine is in [journal/260830-chart-engine.md](../journal/260830-chart-engine.md); this branch's own work is in [journal/260830-spending-view-backend.md](../journal/260830-spending-view-backend.md).) Earlier the same day — §10.1 now enumerates **five** writers of `amount_total`, not four — Ask's document-edit tool is the fifth, and the only one that does not translate the trigger's refusal into an answer the owner can act on; §13 gains that limit and the two adjacent ones found in the same review, re-extraction's partial skip of `amount_total` while still writing `currency`, and `skipped_fields` landing outside the `extraction_completed` event detail. The live-archive verification of this engine is [journal/260830-chart-engine.md](../journal/260830-chart-engine.md). Earlier (2026-08-29) — initial version — the `spend_facts` relation and the canonical-document rule, the `spend_lines`/`line_labels` write path and its two deferred sum triggers, rule translation, the two orthogonal axes and the invariant total, per-document-date conversion, the footer's **eight** categories, the drill-through, LLM rule drafting against the closed vocabulary, the ten `/api/spending` + `/api/documents/{id}/spend-lines` routes, and the `line_labels` index measured and declined. Design: [superpowers/specs/2026-08-28-charts-redesign-design.md](superpowers/specs/2026-08-28-charts-redesign-design.md) §5, §8.4 and §9; plan: [superpowers/plans/2026-08-29-charts-engine.md](superpowers/plans/2026-08-29-charts-engine.md). What an amount *means* is [money-facts.md](money-facts.md); this document is what a chart does with it.)
**Last verified:** 2026-09-02 — method: partial, scoped to the follow-up. The render defect was **reproduced before being fixed**: removing the new `message` block from the approximation branch — i.e. restoring exactly what shipped in #156 — reds `explains WHY a term was dropped on a partial draft, not just which`, and nothing else. The `"007"` correction rests on reading `_CELL_NARROWING` (`CAST(sf.sender_id AS text) IS NOT DISTINCT FROM CAST(:split_value AS text)`) against `_resolve_splits`' parsed-id keying, and on the pinning test asserting only `label` and never `payments`/`total` — that asymmetry is the evidence, and it is why the claim was narrowed rather than the behaviour changed. The empty-string refusal and the `unmatchable_terms` de-duplication and union cap are each pinned by a test. `/footer`'s `amount_kind` gap was read off `api/spending.py`'s bucket check and `footer.py`'s `row.amount_kind == amount_kind` comparison; it is **not** fixed here and is recorded as tracked. Nothing else on this page was re-checked. Earlier the same day — method: partial, scoped to the #127 changes, and every behavioural claim confirmed by **reverting the fix and watching the test fail**, not by reading. `filter_drafted_rule`'s drop: neutered the `claimed_by_in` branch and both new pure tests reded; separately **implemented the intersection merge #127 proposed** and `test_the_second_in_clause_is_dropped_not_merged_into_the_first` reded while the sibling stayed green, which is why both exist. The end-to-end shape is pinned by two route tests driving the stubbed model, both confirmed to red with the drop reverted — one asserting the preview is a real `60.00` rather than the old all-zero chart, one that the drafted rule now saves with a 201 instead of the 422 it used to hit. `/cell`: the unsplit branch and the sender-id parse were each neutered in turn and reded their own tests and no others. The narrowness of both guards is pinned by accepted-halves — two `not_in` clauses and a mixed pair both survive the draft; `"+5"`, fullwidth and Arabic-indic digits still resolve, and `"007"` still names its sender — which is what stops an over-broad fix passing. Four previously-200 assertions in `_MALFORMED_SPLIT_VALUES` were rewritten deliberately into a refused/accepted pair; the property that list was built for, that none of them may 500, still holds. Full `tests/test_api_spending.py` and `tests/test_chart_draft.py` green (128). The two §13 limits removed for `extraction/apply.py` are verified in `docs/ingestion.md`'s stamp of the same date (that file is where the code lives; note `charts.md`'s `Covers:` does NOT include it, so this page's gate could not have caught a stale claim either way). The `secondary()` trap was confirmed by **implementing** the obvious fix — a `skipped_fields` branch in `secondary()` with the template block removed — and watching two vitest cases red while the component still compiled and rendered nothing. Frontend: `npm run type-check`, `npm run lint` and 1445 vitest tests green. **The render claim rests on a render, not on vitest**: the local stack was brought up, a document given an `extraction_completed` event carrying `skipped_fields: [amount_total, currency, amount_kind]`, and its detail page opened in Chromium — the curated timeline shows `Left unchanged: Amount, Currency, Amount kind` in amber between the method line and the chips, without expanding "Show all events", in both light and dark themes. jsdom cannot establish that, which is why it was done in a browser. Nothing else on this page was re-checked. Earlier the same day — method: partial, scoped to §2's readers claim, and checked as an enumeration rather than by re-reading the sentence. `grep -rln 'spend_facts' src/` returns `spend_lines.py`, `models.py`, `charts/query.py`, `charts/rule.py`, `charts/footer.py`, `api/facets.py`, `api/spending.py` and `structured_query.py`; each was opened to separate the modules that merely *name* the view from those that SELECT against it, which is what the claim is about. Counting `FROM|JOIN spend_facts` per file gives four readers — `structured_query.py`, `charts/query.py`, `charts/footer.py`, `api/facets.py` — and zero in `api/spending.py`, `charts/rule.py`, `spend_lines.py` and `models.py`, which name the view without querying it. That enumeration is what caught the first draft of this edit asserting no router builds SQL against the view: `api/facets.py` is a router and does, so the claim was narrowed to `api/spending.py` specifically, which the count confirms. Nothing else on this page was re-checked; the rest carries forward the verification below. Earlier: 2026-09-01 — method: partial, scoped to §3's model block, §9, §11 and §13's two new limits. Route count re-derived rather than trusted: `grep -c '@router\.\(get\|post\|patch\|delete\|put\)' src/library/api/spending.py` returns 13, matching §11's table and this module's own docstring. Read the new `preview_rule` handler in full and confirmed it calls `_validate_rule`, `_validate_split` and `_answer(session, None, query)` and reaches no model — the last pinned by `test_previewing_does_not_call_the_model`, which leaves `stub_anthropic` unconfigured so any call raises. Dropping the `_validate_rule` line was observed to turn `test_previewing_an_unknown_facet_is_a_422_naming_it` from red to a 200 with an empty answer, which is what §11's claim rests on. §3's block is byte-compared against `src/library/charts/rule.py`. §13's `question_text` limit read off the `PATCH` handler's two independent branches and pinned by `test_patching_a_rule_leaves_question_text_untouched`; the same-facet refusal read off `_refuse_unmatchable_conjunction`, and its "still loadable" clause verified by `test_a_chart_with_two_in_clauses_on_one_facet_still_loads_by_id`. §§1-2, 4-8, 10 and 12 were not re-checked and carry forward unchanged.
...
(migration `0035`), and the chart engine's own SQL against it lives in
`src/library/charts/query.py` and `src/library/charts/footer.py`.

The view is **not** exclusive to the chart engine, and reading this section as
though it were is how a second reader gets written by accident. Four modules
issue SQL against it, deliberately: the two above, plus
`src/library/api/facets.py` (label counts, §1.23.5 of [api.md](api.md)) and
`structured_query.sum_amount` (Ask's money totals, #136 — see
[ask.md](ask.md) §1.2, *Where a money total comes from*).

What is exclusive is narrower, and it is the property the invariants below
actually rest on: **`api/spending.py`, the chart engine's own router, builds no
SQL at all** — it parses, validates, calls this package and serialises. So every
query behind a chart goes through `charts/query.py`, and the drill-through
provably sums the rows the chart summed. `api/facets.py` is a router that does
write its own SQL against the view, and that is not a violation of this rule: it
answers a different question (what the empty state can propose a chart from) and
holds none of the invariants below.
...
exactly one. Omitting it for `bucket=excluded` is a `422` naming the
requirement, not an empty page: every `excluded` row carries a kind, so
`row.amount_kind == amount_kind` can never match a missing or unrecognised
one, and that would otherwise be indistinguishable from the group genuinely
being empty.
...
| --- | --- | --- |
| the draft flow (§9.1) | `filter_drafted_rule` | **drops it** and reports it — as `unknown_terms` when the vocabulary does not contain it, or as `unmatchable_terms` when it does but the clause cannot be combined |
| the rule editor (§11) | `_validate_rule` | **422 naming it**; nothing is saved |

The two report lists are kept apart on purpose. A second `in` clause on a facet
names only vocabulary that exists — it is dropped because a document carries at
most one value per facet, not because anything in it is unknown — so folding it
into `unknown_terms` would print "not in the vocabulary" about values that are.
The API merges both into one wire field for display but composes its `message`
from the two causes separately, so the reason a reader is given is the reason
that applies.
...
Everything dropped is returned in the wire field `unknown_terms`, capped in
count and length (it is unbounded model-authored text that will be rendered). A
blank or whitespace-only term is reported as `(blank)` rather than dropped
silently: an unreported drop is a silently narrowed question.

**One wire list, two causes.** `DraftResult` keeps them apart internally —
`unknown_terms` for what the vocabulary does not contain, `unmatchable_terms`
for a clause whose every value *is* in the vocabulary but which cannot be
combined (§9) — and the route concatenates them into the single response field,
because the client renders the list as neutral chips. The distinction is
carried by `message`, which is composed from the two causes separately: only
the first is introduced as "not in the vocabulary", the second as "could not be
combined, since a document takes at most one value per facet". Merging the
*message* as well would print a false reason for a real drop, which is the
failure §12 is about one layer down. `expressible` and the collapsed branch key
on the union, so either cause alone makes a draft an approximation.
...
because a refusal only helps if the person reading it can act on it. All five
now answer.
...
| Ask's document-edit tool (`ask/engine.py`) | a tool-result `error` naming the allocation and saying to clear or replace the lines first — literally the same wording the `PATCH` route gives (both pass `spend_lines.AMOUNT_ALLOCATED_REFUSAL`), delivered where Ask can act on it |

The fifth row was **unguarded until 2026-08-31**. `_WRITABLE_FIELDS` is
`tuple(DocumentUpdate.model_fields)`, which includes `amount_total`, and the tool
committed with a plain `session.commit()`, so the trigger's `DBAPIError` escaped
the whole Ask turn as a 500 with a poisoned session. It now commits through the
same `spend_lines.commit_allocation` helper as the other guarded writers and
returns the refusal as a tool result, so the model can relay it or clear the
lines and retry. The defect was never observed: it is unreachable until the
archive holds its first allocated document, of which there are none.
...
`/charts` is the name the redesign replaced — the routes under it answered
"what did this one `(sender, kind, currency)` series do over time", which is not
the question this engine answers — so moving here would import a name for the
thing that was wrong. The SPA route stays `/charts` because that is the URL in
the user's muscle memory and their bookmarks; the API/frontend asymmetry is
...
so a drilled panel can title itself without a second read of `/data`. For an
**unsplit** chart `_resolve_splits` returns `[]` (same as `DataOut.splits`
above), so `chart_cell_data` has no resolved bucket to read `label` from and
it falls back to `""` — not a placeholder string — which the panel should
read as "this chart has no split axis to title itself with," not render as
a blank title.
...
- **Two `in` clauses on one facet are a 422**, on the same four paths — the
  ...
  **`POST /spending/draft` is the fifth path, and it answers differently.** It
  does not call `_validate_rule` — `filter_drafted_rule` guarantees vocabulary
  membership by construction — and that guarantee says nothing about whether the
  surviving clauses can be *combined*. So until 2026-09-02 a drafted same-facet
  pair reached the client as `expressible: true` with a 200 preview of an
  all-zero chart, and then 422'd the moment the owner pressed Save. The filter
  now **drops the second `in` clause and reports it**, which is what every other
  undraftable input on that path already gets (§9): the draft's contract is to
  answer what it can and say what it dropped, and a 422 on a plain-language
  question is not the answer that surface exists to give. Dropped, still not
  merged — narrowing, never widening.
...
- **`/cell` refuses a `split_value` that names no possible bucket** — a 422,
  naming the value, in the two cases decidable from the split axis alone: any
  non-null value on an **unsplit** chart (whose split expression is
  `CAST(NULL AS text)`, so nothing can match), and a value that is not a
  `senders.id`-shaped integer under `split=sender`. The same class the `period`
  check fixes on the same route: an unmatchable argument used to answer
  `total: "0.00"` with no payments, which reads as "you spent nothing here".
  **Membership is deliberately not checked.** A facet value can be deleted while
  `spend_facts.labels` still carries it, so that bucket *is* drawn by `/data`
  and refusing it would reject a cell the chart just rendered; and a sender that
  contributed nothing to the window is an honest empty bucket. Existence of the
  value and existence of the bucket are independent in both directions, so only
  the shape is safe to judge here. The sender check **parses** rather than
  pattern-matches, so non-canonical spellings of a real id (`"007"`, `"+7"`,
  fullwidth digits) are not refused. Note what that does and does not buy:
  `_resolve_splits` keys on the parsed id, so such a value resolves to the
  sender's **name**, but `chart_cell` binds the **raw** string into
  `CAST(sf.sender_id AS text) IS NOT DISTINCT FROM CAST(:split_value AS text)`,
  and `"7"` does not equal `"007"` — so the panel comes back correctly
  labelled and **empty**. The guard therefore admits a narrow class of the
  silence it exists to remove. The exact test is canonical form
  (`split_value == str(int(split_value))`), which would refuse these; it is not
  applied because a pre-existing test pins the label-resolution behaviour and
  changing it is a deliberate decision rather than a tightening. Tracked in
  issue 158. `?split_value=` (the empty string) **is** refused, on every axis.
  Also unvalidated on the sibling route: `/footer`'s `amount_kind` is never
  checked against `AmountKind`'s members, so `?bucket=excluded&amount_kind=bogus`
  returns an empty bucket rather than a 422 — the same class, also issue 158.
...
- **Two `in` clauses on one facet** are ANDed into a permanently empty chart,
  ...
  **The state above is no longer reachable for a rule saved from now on**, and
  ...
  `POST /spending/draft` is a fifth path and is the exception: it does not call
  `_validate_rule` at all, and since 2026-09-02 `filter_drafted_rule` drops the
  second `in` clause and reports it instead of refusing (§11). Before that it
  did neither, so a drafted pair previewed as an all-zero chart and refused only
  at Save.
...
- ~~**Re-extraction's skip is partial.**~~ **Fixed 2026-09-02.** It skipped
  `amount_total` on an allocated document but still wrote `currency` and
  `amount_kind` through the same `scalar_values` loop, so a re-read finding a
  different currency left the currency disagreeing with the denomination of the
  lines — and `amount_currency_coupling` could not fire, being an XOR on
  *presence* with both fields set. All three are now withheld together
  (`_ALLOCATION_LOCKED_FIELDS`). The skip withholds a **change**, not a fill: a
  document whose currency was never extracted still gains one, since filling a
  blank contradicts no allocation.
- ~~**`skipped_fields` is recorded where nobody reads it.**~~ **Fixed
  2026-09-02.** It landed in `extra["extraction"]` only, and not in the
  `extraction_completed` event detail — the surface the document timeline
  renders — so the owner was told nothing about the field left behind. It is now
  in the event detail (present only when something was withheld) *and* rendered
  as its own amber line in the curated timeline.

  The second half was not optional. The timeline renders **no** field list at
  all — it never showed `fields_set` either — so the event-detail change alone
  would have surfaced a withheld write only inside the collapsed "Show all
  events" JSON dump, among eight other keys. Note also that
  ...
  for this event type: a branch added there compiles, passes a unit test that
  calls `secondary()` directly, and renders nothing. Confirmed by trying it.
