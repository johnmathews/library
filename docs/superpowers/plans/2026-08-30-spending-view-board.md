...
- **The surface colour is theme-dependent.** Read it from the rendered surface rather than hardcoding white in both modes: `#ffffff` light, `#1f2937` dark (`gray-800`, the `.card` background). Derive from `document.documentElement.classList.contains('dark')` at render and re-render on the theme toggle, the way the app's other themed canvas work does.
