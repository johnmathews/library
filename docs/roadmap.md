...
- **The chart rule editor** (2026-09-01, issue #135): a saved chart's rule is
  ...
  **`display_currency` and `default_grain`** are accepted by `PATCH` but nothing
  persists them — the workspace toolbar's grain and currency controls are view
  state for the session, not the chart's saved defaults — and **`question_text`**
  is deliberately never rewritten by a rule edit
