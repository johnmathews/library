...
| `no_text_extracted` | (document) | `ocr_text` is empty or whitespace-only **and** extraction did not read the page image (`input_mode` `document`/`image` — the same guard as `amount_grounding`, so a vision-rescued photo is not nagged for the case the fallback exists to handle). The document still reaches `indexed` — that invariant is deliberate, the original is stored and OCR can be re-run — but with no text it is invisible to search and to Ask, so it must not pass silently. **Also seeded at the OCR stage** (`jobs.run_ocr` → `flag_textless_document`, plus an `ocr_empty` event), because a textless document normally takes extraction's `ExtractionSkipped("input_unusable")` branch and every skip path returns before validation runs — so relying on extraction's validation would leave the one document that most needs flagging unflagged. The OCR-stage seed is a floor: it is skipped when a `validation` payload already exists, so a pipeline resume cannot overwrite a richer finding set with this single one. |
