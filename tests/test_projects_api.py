...
def test_create_list_get_with_counts(api_client: TestClient, api_database_url: str) -> None:
    project = create_project(api_client, "W6 Kitchen Reno", description="renovation papers")
    ...
    one = api_client.get("/api/projects/w6-kitchen-reno")
...
def test_create_explicit_slug_is_normalised(api_client: TestClient) -> None:
    project = create_project(api_client, "W6 Explicit", slug="W6 Custom Slug!!")
    assert project["slug"] == "w6-custom-slug"


def test_create_duplicate_slug_409(api_client: TestClient) -> None:
    create_project(api_client, "W6 Dup")
    response = api_client.post("/api/projects", json={"name": "W6 Dup"})
...
def test_patch_updates_name_description_and_archive(api_client: TestClient) -> None:
    create_project(api_client, "W6 Patch Me", description="before")
    ...
    archived = api_client.patch("/api/projects/w6-patch-me", json={"archived": True})
    assert archived.status_code == 200
    assert archived.json()["archived"] is True

    default_slugs = [p["slug"] for p in api_client.get("/api/projects").json()]
    assert "w6-patch-me" not in default_slugs
    all_slugs = [
        p["slug"] for p in api_client.get("/api/projects", params={"include_archived": True}).json()
    ]
    assert "w6-patch-me" in all_slugs

    # Unarchiving restores it.
    restored = api_client.patch("/api/projects/w6-patch-me", json={"archived": False})
    assert restored.json()["archived"] is False
    assert "w6-patch-me" in [p["slug"] for p in api_client.get("/api/projects").json()]


def test_patch_unknown_404(api_client: TestClient) -> None:
    assert api_client.patch("/api/projects/w6-nope", json={"name": "x"}).status_code == 404


def test_delete_removes_memberships_but_not_documents(
    api_client: TestClient, api_database_url: str
) -> None:
    create_project(api_client, "W6 Delete Me")
    document_id = seed_document(
        api_database_url, "w6-proj-delete-doc", project_slugs=["w6-delete-me"]
    )

    assert api_client.delete("/api/projects/w6-delete-me").status_code == 204

    # Project gone, but the document survives with no membership.
    assert api_client.get("/api/projects/w6-delete-me").status_code == 404
    doc = api_client.get(f"/api/documents/{document_id}")
...
def test_delete_unknown_404(api_client: TestClient) -> None:
    assert api_client.delete("/api/projects/w6-nope-delete").status_code == 404
